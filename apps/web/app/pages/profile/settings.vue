...
<template>
  <section class="min-h-screen bg-gradient-to-br from-amber-50 via-white to-sky-100 text-slate-900 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 dark:text-slate-100">
    <div class="mx-auto max-w-5xl px-4 py-10">
      <header class="mb-8">
        <p class="text-sm uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">Profilo</p>
        <h1 class="mt-2 text-3xl font-serif font-semibold">Impostazioni account</h1>
        <p class="mt-2 text-slate-600 dark:text-slate-300">Completa il profilo, verifica l'email e scegli il tipo di 2FA.</p>
      </header>

      <div class="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div class="space-y-6">
          <div class="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
            <h2 class="text-lg font-semibold">Dati personali</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">Aggiorna i dati richiesti dal profilo.</p>

            <div class="mt-6 grid gap-4 sm:grid-cols-2">
              <div>
                <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Nome completo</label>
              ...
              <div>
                <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Data di nascita</label>
                <input v-model="form.birthDate" type="date"
                  class="mt-2 w-full rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-100 dark:focus:border-slate-500" />
              </div>
            </div>
            <div class="mt-4">
              <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Biografia</label>
              <textarea v-model="form.bio" rows="4" class="mt-2 w-full rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-slate-500" placeholder="Racconta qualcosa di te..."></textarea>
            </div>
          </div>

          <div class="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
            <h2 class="text-lg font-semibold">Preferenze di lettura</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">Personalizza tema, lingua e dimensione testo.</p>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-3">
              ...
              <div>
                <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Lingua UI</label>
              ...
              <div>
                <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Dimensione testo</label>
                <input v-model.number="form.fontSize" type="number" min="12" max="28"
                  class="mt-2 w-full rounded-xl border border-slate-200 bg-white/70 px-3 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-100 dark:focus:border-slate-500" />
              </div>
              <div>
                <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Dimensione immagini</label>
            ...
            <div class="mt-4 space-y-2">
              <label class="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-200">
                <input type="checkbox" v-model="form.accountPublic" :true-value="1" :false-value="0" class="w-4 h-4" />
                <span>Account pubblico (visibile nelle ricerche)</span>
              </label>
              <label class="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-200">
                <input type="checkbox" v-model="form.listsPublicByDefault" :true-value="1" :false-value="0" class="w-4 h-4" />
        ...
        <div class="space-y-6">
          <div class="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
            ...
            <div class="mt-4 flex items-center gap-2 text-sm">
              <span class="inline-flex items-center rounded-full px-3 py-1"
                :class="isEmailVerified ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'">
                {{ isEmailVerified ? 'Verificata' : 'Da verificare' }}
              </span>
              <span v-if="emailVerifiedAt" class="text-slate-500 dark:text-slate-400">{{ emailVerifiedAt }}</span>
            </div>
          ...
          <div class="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">Scegli tra email o app di autenticazione gratuita.</p>
            ...
            <div class="mt-4 space-y-2">
              <button type="button" @click="setTwoFactorMethod('none')"
                ...
                :class="twoFactorMethod === 'none' ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950' : ''">
                Nessuna 2FA
              </button>
              <button type="button" @click="setTwoFactorMethod('email')"
                class="w-full rounded-full border border-slate-200 px-4 py-2 text-left text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                :class="twoFactorMethod === 'email' ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950' : ''">
                2FA via email
              </button>
              <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950/70">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Google Authenticator, Microsoft Authenticator, Authy.</p>
                  </div>
                ...
                <div v-if="totpQrCode" class="mt-4 space-y-3">
                  <img :src="totpQrCode" alt="QR TOTP" class="h-40 w-40 rounded-xl bg-white p-2 dark:bg-slate-100" />
                  <p class="text-xs text-slate-500 dark:text-slate-400">Codice manuale: {{ totpSecret }}</p>
            ...
            <p v-if="twoFactorMessage" class="mt-3 text-sm text-emerald-700 dark:text-emerald-300">{{ twoFactorMessage }}</p>
            <p v-if="twoFactorError" class="mt-3 text-sm text-rose-700 dark:text-rose-300">{{ twoFactorError }}</p>
            <p v-if="totpEnabledAt" class="mt-2 text-xs text-slate-500 dark:text-slate-400">Attivo dal: {{ totpEnabledAt }}</p>
          </div>
        </div>
      </div>
    </div>
