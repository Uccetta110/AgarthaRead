<template>
  <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
    ...
    <p class="mt-2 text-slate-600 dark:text-slate-300">Ultime e notizie in evidenza.</p>

    <div v-if="data?.value?.notice" class="mt-3 text-sm text-yellow-600 dark:text-yellow-300">{{ data.value.notice }}</div>
